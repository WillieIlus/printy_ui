...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<aside${ssrRenderAttrs(mergeProps({ class: "softui-panel floating-form-panel sticky top-24 flex max-h-[calc(100vh-7rem)] min-h-0 flex-col overflow-hidden rounded-lg" }, _attrs))}><div class="flex items-start justify-between gap-4 border-b border-white/8 px-4 py-3.5 sm:px-5"><div><p class="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--p-text-muted)]">Editor</p><h2 class="mt-1 text-base font-semibold text-[var(--p-text)] sm:text-lg">${ssrInterpolate(__props.title)}</h2>`);
      if (__props.description) {
        _push(`<p class="mt-1 text-sm leading-5 text-[var(--p-text-muted)]">${ssrInterpolate(__props.description)}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
      ...
      _push(`</div><div class="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5">`);
...
//# sourceMappingURL=AdminWorkspaceFormPanel-D_9BQu_x.mjs.map
