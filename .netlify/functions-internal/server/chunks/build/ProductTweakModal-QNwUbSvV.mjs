import { w as useApi, u as usePublicApi, a as _sfc_main$f, c as _sfc_main$9, A as API } from './server.mjs';
import { _ as _sfc_main$1 } from './Badge-CGrQBVmm.mjs';
import { _ as _sfc_main$2 } from './Checkbox-BX22gW7J.mjs';
import { _ as _sfc_main$3 } from './Textarea-GjRObXeo.mjs';
import { defineComponent, computed, useModel, ref, reactive, watch, unref, withCtx, createTextVNode, toDisplayString, createVNode, mergeModels, useSSRContext } from 'vue';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    function resetForm() {
      ...
      const defaultFinishings = (props.product.finishing_options ?? []).filter((o) => o.is_default).map((o) => ({ finishing_rate: o.finishing_rate }));
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (isOpen.value) {
          _push2(`<div class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true"><div class="absolute inset-0 bg-black/50 backdrop-blur-sm"></div><div class="modal-panel relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-[var(--p-surface)] rounded-2xl shadow-2xl z-10"><div class="sticky top-0 z-10 bg-[var(--p-surface)]/95 backdrop-blur-sm border-b border-[var(--p-border)] px-6 py-4 rounded-t-2xl"><div class="flex items-start justify-between gap-4"><div><h2 class="text-lg font-bold text-[var(--p-text)]"> Tweak Quote — ${ssrInterpolate(__props.product.name)}</h2><p class="text-sm text-[var(--p-text-muted)] mt-0.5"> Customize paper, quantity, finishing, and other options before adding to your quote. </p></div><button class="rounded-lg p-1.5 text-[var(--p-text-muted)] hover:text-[var(--p-text)] hover:bg-[var(--p-surface-sunken)] transition-colors">`);
          ...
          if (unref(successMessage)) {
            ...
            if (__props.product.price_hint?.can_calculate || __props.product.price_range_est?.can_calculate) {
              ...
              if (unref(tweakPriceSummary)) {
                _push2(`<div class="flex justify-between items-baseline"><span class="font-semibold text-[var(--p-text)]">Est. total</span><span class="text-lg font-bold text-flamingo-600 dark:text-flamingo-400">${ssrInterpolate(unref(tweakPriceSummary).totalLine)}</span></div>`);
              } else {
                _push2(`<!---->`);
              }
...
//# sourceMappingURL=ProductTweakModal-QNwUbSvV.mjs.map
