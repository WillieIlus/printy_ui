...
import { g as useRoute, i as useToast, d as _sfc_main$9, a as _sfc_main$f, x as useNuxtApp, A as API } from './server.mjs';
import { _ as __nuxt_component_4 } from './SkeletonState-BhcHA7h3.mjs';
import { _ as __nuxt_component_4$1 } from './SectionCard-BarCx775.mjs';
import { _ as _sfc_main$4 } from './Badge-xP9ZTQag.mjs';
import { _ as __nuxt_component_6 } from './RichTextDisplay-BjJZ3jTm.mjs';
import { _ as _sfc_main$5 } from './Card-HKq-lRAa.mjs';
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_UCard, _attrs, {
        header: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<h3 class="text-lg font-semibold text-gray-900 dark:text-white"${_scopeId}>Quote items</h3>`);
          } else {
            return [
              createVNode("h3", { class: "text-lg font-semibold text-gray-900 dark:text-white" }, "Quote items")
            ];
          }
        }),
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (__props.items?.length) {
              ...
              ssrRenderList(__props.items, (item) => {
                _push2(`<div class="flex justify-between items-center rounded-lg border dark:border-gray-700 p-3"${_scopeId}><div${_scopeId}><p class="font-medium text-gray-900 dark:text-white"${_scopeId}>${ssrInterpolate(item.name)}</p><p class="text-sm text-gray-500 dark:text-gray-400"${_scopeId}>Qty ${ssrInterpolate(item.quantity)} × ${ssrInterpolate(item.unit_price)}</p></div><p class="font-semibold text-gray-900 dark:text-white"${_scopeId}>${ssrInterpolate(item.total_price)}</p></div>`);
              });
              _push2(`<!--]--></div>`);
            } else {
              _push2(`<p class="text-sm text-gray-500 dark:text-gray-400"${_scopeId}>No items.</p>`);
            }
              ...
              __props.items?.length ? (openBlock(), createBlock("div", {
                ...
                (openBlock(true), createBlock(Fragment, null, renderList(__props.items, (item) => {
                  return openBlock(), createBlock("div", {
                    key: item.id,
                    class: "flex justify-between items-center rounded-lg border dark:border-gray-700 p-3"
                  }, [
                    createVNode("div", null, [
                      createVNode("p", { class: "font-medium text-gray-900 dark:text-white" }, toDisplayString(item.name), 1),
                      createVNode("p", { class: "text-sm text-gray-500 dark:text-gray-400" }, "Qty " + toDisplayString(item.quantity) + " × " + toDisplayString(item.unit_price), 1)
                    ]),
                    createVNode("p", { class: "font-semibold text-gray-900 dark:text-white" }, toDisplayString(item.total_price), 1)
                  ]);
                }), 128))
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    function copyLink() {
      (void 0).clipboard.writeText(props.shareUrl).then(
        () => toast.add({ title: "Link copied", color: "success" }),
        () => toast.add({ title: "Could not copy link", color: "error" })
      );
    }
    function copyWhatsApp() {
      (void 0).clipboard.writeText(props.whatsappText).then(
...
//# sourceMappingURL=_id_-Bym4oaPl.mjs.map
