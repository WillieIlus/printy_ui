import { f as useRoute, _ as __nuxt_component_3$1, a as _sfc_main$f, d as _sfc_main$9 } from './server.mjs';
...
import { f as formatDate } from './formatters-r_qbKRfS.mjs';
import { u as usePublicJob } from './useJobRequests-CStOb5H3.mjs';
import '../_/nitro.mjs';
import 'lru-cache';
import '@unocss/core';
    ...
    return (_ctx, _push, _parent, _attrs) => {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            return [
              ...
              createTextVNode(" Back to Printy ")
            ];
          }
        }),
        _: 1
      ...
      if (unref(loading) && !unref(job)) {
        ...
        if (unref(job).location) {
          _push(`<div class="text-sm"><span class="text-[var(--p-text-muted)]">Location:</span><span class="text-[var(--p-text)] ml-2">${ssrInterpolate(unref(job).location)}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(job).deadline) {
          _push(`<div class="text-sm"><span class="text-[var(--p-text-muted)]">Deadline:</span><span class="text-[var(--p-text)] ml-2">${ssrInterpolate(unref(formatDate)(unref(job).deadline))}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        if (Object.keys(unref(job).specs || {}).length) {
          _push(`<div class="space-y-2"><p class="text-sm font-medium text-[var(--p-text-muted)]">Specs</p><ul class="space-y-1 text-sm"><!--[-->`);
        ...
        _push(`<div class="pt-4 border-t border-[var(--p-border)]"><p class="text-sm text-[var(--p-text-muted)] mb-3">${ssrInterpolate(unref(job).claim_cta ?? "Claim job")}</p>`);
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              return [
                ...
                createTextVNode(" Sign in to claim ")
              ];
            }
          }),
          _: 1
...
//# sourceMappingURL=_token_-VMmX8pb4.mjs.map
