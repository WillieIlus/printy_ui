import { f as useRoute, n as navigateTo, d as _sfc_main$9, _ as __nuxt_component_3$1, a as _sfc_main$f } from './server.mjs';
...
import { _ as _sfc_main$1 } from './SelectMenu-C6Eyp_GI.mjs';
import { _ as _sfc_main$2 } from './Badge-DRRvJchD.mjs';
import { defineComponent, ref, computed, watch, mergeProps, withCtx, createVNode, unref, isRef, createTextVNode, resolveDynamicComponent, toDisplayString, openBlock, createBlock, createCommentVNode, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate, ssrRenderClass, ssrRenderList, ssrRenderVNode, ssrRenderSlot } from 'vue/server-renderer';
import { u as useSellerStore } from './seller-DvrkRojT.mjs';
import { u as useSetupStatus } from './useSetupStatus-DBIzE8qw.mjs';
import { u as useAdminWorkspace } from './useAdminWorkspace-f_blWgUx.mjs';
import '../_/nitro.mjs';
import 'lru-cache';
import '@unocss/core';
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(navSections), (section) => {
        _push(`<section><p class="mb-2 px-3 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--p-text-muted)]">${ssrInterpolate(section.label)}</p><div class="space-y-1"><!--[-->`);
        ssrRenderList(section.items, (item) => {
          ssrRenderVNode(_push, createVNode(resolveDynamicComponent(item.to ? "NuxtLink" : "button"), {
            key: `${section.label}-${item.label}`,
            to: item.to,
            type: "button",
            class: ["flex items-start gap-3 rounded-2xl px-3 py-3 transition", isActive(item.to) ? "bg-[var(--p-surface-sunken)] text-[var(--p-text)] shadow-sm" : "text-[var(--p-text-dim)] hover:bg-[var(--p-surface-sunken)]/70 hover:text-[var(--p-text)]"],
            onClick: ($event) => handleNavItemClick(item)
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
...
//# sourceMappingURL=dashboard-CtjbMO2O.mjs.map
