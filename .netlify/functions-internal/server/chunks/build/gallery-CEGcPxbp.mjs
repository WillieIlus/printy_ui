import { g as useToast, c as _sfc_main$9, i as __nuxt_component_5, a as _sfc_main$f, _ as __nuxt_component_2$1, n as navigateTo } from './server.mjs';
import { _ as __nuxt_component_2 } from './LoadingSpinner-DUXHKoma.mjs';
import { _ as __nuxt_component_3 } from './EmptyState-InVpxbUI.mjs';
...
import { g as getGalleryProductOptions, c as calculateGalleryProductPrice } from './gallery-C6fffNC7.mjs';
import { getActiveDraft, tweakAndAdd } from './quoteDraft-BcBTDVPw.mjs';
        ...
        if (__props.open) {
          ...
          if (pricingMode.value === "SHEET" && papers.value.length) {
            ...
            ssrRenderList(papers.value, (p) => {
              _push2(`<label class="${ssrRenderClass([form.paper === p.id ? "bg-flamingo-50 dark:bg-flamingo-900/20" : "hover:bg-[var(--p-surface-sunken)]", "flex items-center gap-3 rounded-lg px-3 py-2 cursor-pointer transition-colors"])}"><input${ssrIncludeBooleanAttr(ssrLooseEqual(form.paper, p.id)) ? " checked" : ""} type="radio"${ssrRenderAttr("value", p.id)} class="accent-flamingo-500"><span class="text-sm">${ssrInterpolate(p.sheet_size)} ${ssrInterpolate(p.gsm)}gsm ${ssrInterpolate(p.paper_type)}</span><span class="ml-auto text-xs text-[var(--p-text-muted)]">KES ${ssrInterpolate(p.selling_price)}</span></label>`);
            });
            _push2(`<!--]--></div></div>`);
          } else {
            _push2(`<!---->`);
          ...
          if (pricingMode.value === "LARGE_FORMAT" && materials.value.length) {
            ...
            ssrRenderList(materials.value, (m) => {
              _push2(`<label class="${ssrRenderClass([form.material === m.id ? "bg-flamingo-50 dark:bg-flamingo-900/20" : "hover:bg-[var(--p-surface-sunken)]", "flex items-center gap-3 rounded-lg px-3 py-2 cursor-pointer transition-colors"])}"><input${ssrIncludeBooleanAttr(ssrLooseEqual(form.material, m.id)) ? " checked" : ""} type="radio"${ssrRenderAttr("value", m.id)} class="accent-flamingo-500"><span class="text-sm">${ssrInterpolate(m.material_type ?? "Material")}</span><span class="ml-auto text-xs text-[var(--p-text-muted)]">KES ${ssrInterpolate(m.selling_price)}/${ssrInterpolate(m.unit)}</span></label>`);
            });
            _push2(`<!--]--></div></div>`);
          } else {
            _push2(`<!---->`);
          ...
          if (finishingRates.value.length) {
            ...
            ssrRenderList(finishingRates.value, (fr) => {
              ...
              _push2(`<span class="text-sm flex-1">${ssrInterpolate(fr.name)}</span><span class="text-xs text-[var(--p-text-muted)]">KES ${ssrInterpolate(fr.price)}</span></label>`);
            });
            _push2(`<!--]--></div></div>`);
          } else {
            _push2(`<!---->`);
          ...
          if (calculating.value) {
            ...
            _push2(`<!--[--><div class="flex justify-between items-baseline"><span class="font-semibold text-[var(--p-text)]">Suggested price</span><span class="text-xl font-bold text-flamingo-600 dark:text-flamingo-400">${ssrInterpolate(unref(formatKES)(totalPrice.value))}</span></div><div class="border-t border-flamingo-200/60 dark:border-flamingo-800/30 pt-3 mt-3"><table class="w-full text-sm"><tbody class="divide-y divide-[var(--p-border-dim)]"><!--[-->`);
            ssrRenderList(breakdownRows.value, (row) => {
              _push2(`<tr><td class="py-1.5 text-[var(--p-text-muted)]">${ssrInterpolate(row.label)}</td><td class="py-1.5 text-right font-medium">${ssrInterpolate(unref(formatKES)(row.amount))}</td></tr>`);
            });
            _push2(`<!--]--></tbody></table><div class="flex justify-between pt-2 mt-2 border-t border-[var(--p-border)]"><span class="font-semibold">Total</span><span class="font-bold text-flamingo-600 dark:text-flamingo-400">${ssrInterpolate(unref(formatKES)(totalPrice.value))}</span></div></div>`);
            if (isDemoMode.value) {
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_CommonEmptyState = __nuxt_component_3;
...
//# sourceMappingURL=gallery-CEGcPxbp.mjs.map
