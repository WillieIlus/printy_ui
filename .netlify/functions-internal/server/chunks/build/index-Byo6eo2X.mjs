    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(unref(items), (item) => {
          _push(`<tr class="bg-[var(--p-surface)]"><td class="px-4 py-4 text-sm font-medium text-[var(--p-text)]">${ssrInterpolate(item.sheet_size)}</td><td class="px-4 py-4 text-sm text-[var(--p-text-muted)]">${ssrInterpolate(item.paper_type)}</td><td class="px-4 py-4 text-sm text-[var(--p-text-muted)]">${ssrInterpolate(item.gsm)} gsm</td><td class="px-4 py-4 text-right text-sm text-[var(--p-text)]">${ssrInterpolate(unref(formatMoney)(item.selling_price))}</td><td class="px-4 py-4"><div class="flex justify-end gap-2">`);
      ...
      _push(`</div>`);
      if (unref(panelOpen)) {
        _push(`<div id="paper-form" tabindex="-1">`);
...
//# sourceMappingURL=index-Byo6eo2X.mjs.map
