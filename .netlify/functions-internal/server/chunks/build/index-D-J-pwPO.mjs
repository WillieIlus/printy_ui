...
import { u as useQuoteRequests } from './useQuoteRequests-P1_E3-Z-.mjs';
import '../_/nitro.mjs';
import 'lru-cache';
import '@unocss/core';
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (__props.request.latest_sent_quote?.total) {
              _push2(`<span class="text-sm font-semibold text-[var(--p-text)]"${_scopeId}>${ssrInterpolate(unref(formatMoney)(__props.request.latest_sent_quote.total, __props.request.shop_currency))}</span>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div>`);
              ...
              createVNode("div", { class: "flex justify-between items-start gap-3" }, [
                createVNode("div", { class: "min-w-0 flex-1" }, [
                  ...
                  createVNode("div", { class: "mt-2 flex items-center gap-2" }, [
                    ...
                    }, toDisplayString(unref(formatMoney)(__props.request.latest_sent_quote.total, __props.request.shop_currency)), 1)) : createCommentVNode("", true)
                  ])
                ]),
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const statusTabs = [
      ...
      { value: "awaiting_reply", label: "Need reply" },
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-[var(--p-bg)]" }, _attrs))}><div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8"><div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6"><div><h1 class="text-2xl font-bold text-[var(--p-text)]">My Requests</h1><p class="mt-1 text-sm text-[var(--p-text-muted)]"> Track each request from submission through clarification, rejection, and quote delivery. </p></div><div class="flex gap-2">`);
...
//# sourceMappingURL=index-D-J-pwPO.mjs.map
