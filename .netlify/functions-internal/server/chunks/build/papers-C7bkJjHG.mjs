...
import { d as _sfc_main$9, a as _sfc_main$f, n as navigateTo } from './server.mjs';
...
import { a as listPapersBySlug } from './seller-B-6aM_bM.mjs';
import 'reka-ui';
import '../_/nitro.mjs';
import 'lru-cache';
import '@unocss/core';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "space-y-6" }, _attrs))}><div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div><h1 class="text-2xl font-bold text-[var(--p-text)]">Papers</h1><p class="text-sm text-[var(--p-text-muted)]">Manage paper stock across your shops.</p></div><div class="flex items-center gap-3">`);
      ...
      if (!unref(sellerStore).shops.length && !unref(sellerStore).loading) {
        ...
        _push(`<p class="mt-2 text-sm font-medium text-[var(--p-text-dim)]">No shops yet</p><p class="mt-1 text-sm text-[var(--p-text-muted)]">Create a shop first to manage papers.</p>`);
        _push(ssrRenderComponent(_component_UButton, {
          color: "primary",
          class: "mt-4",
        ...
        _push(`<div class="rounded-xl border border-[var(--p-border)] overflow-hidden"><table class="min-w-full divide-y divide-[var(--p-border)]"><thead class="bg-[var(--p-surface-sunken)]"><tr><th class="px-4 py-3 text-left text-xs font-medium text-[var(--p-text-muted)] uppercase">Size</th><th class="px-4 py-3 text-left text-xs font-medium text-[var(--p-text-muted)] uppercase">Type</th><th class="px-4 py-3 text-center text-xs font-medium text-[var(--p-text-muted)] uppercase">GSM</th><th class="px-4 py-3 text-right text-xs font-medium text-[var(--p-text-muted)] uppercase">Selling price</th><th class="px-4 py-3 text-center text-xs font-medium text-[var(--p-text-muted)] uppercase">Status</th></tr></thead><tbody class="divide-y divide-[var(--p-border-dim)]"><!--[-->`);
        ssrRenderList(unref(items), (p) => {
          _push(`<tr class="hover:bg-[var(--p-surface-sunken)]/50"><td class="px-4 py-3 text-sm font-medium text-[var(--p-text)]">${ssrInterpolate(p.sheet_size)}</td><td class="px-4 py-3 text-sm text-[var(--p-text-muted)]">${ssrInterpolate(p.paper_type)}</td><td class="px-4 py-3 text-center text-sm text-[var(--p-text-muted)]">${ssrInterpolate(p.gsm)}</td><td class="px-4 py-3 text-right text-sm text-[var(--p-text-muted)]">${ssrInterpolate(p.selling_price)}</td><td class="px-4 py-3 text-center">`);
        ...
        _push(`<p class="mt-2 text-sm font-medium text-[var(--p-text-dim)]">No papers yet</p><p class="mt-1 text-sm text-[var(--p-text-muted)]">Add paper stock to your shop.</p>`);
        _push(ssrRenderComponent(_component_UButton, {
          color: "primary",
          class: "mt-4",
          onClick: goToShopSetup
...
//# sourceMappingURL=papers-C7bkJjHG.mjs.map
