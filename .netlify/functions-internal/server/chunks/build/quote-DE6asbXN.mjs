...
const useQuoteStore = defineStore("quote", () => {
  ...
  async function fetchProducts(shopSlug) {
    loading.value = true;
    error.value = null;
    try {
      const { $api } = useNuxtApp();
      products.value = await $api(API.shopProducts(shopSlug));
  ...
  async function createProduct(shopSlug, data) {
    loading.value = true;
    error.value = null;
    try {
      const { $api } = useNuxtApp();
      const product = await $api(API.shopProducts(shopSlug), {
        method: "POST",
        body: data
      });
...
//# sourceMappingURL=quote-DE6asbXN.mjs.map
