import { f as useRoute, e as useAuthStore$1, g as useToast, c as _sfc_main$9, a as _sfc_main$f, n as navigateTo, u as usePublicApi, A as API } from './server.mjs';
import { _ as __nuxt_component_2 } from './LoadingSpinner-DUXHKoma.mjs';
import { defineComponent, computed, ref, mergeProps, withCtx, createVNode, createTextVNode, unref, isRef, toDisplayString, reactive, watch, useModel, mergeModels, useSSRContext } from 'vue';
...
import { previewPrice } from './quoteDraft-BcBTDVPw.mjs';
import { s as setInterval } from './interval-DiSDr_Za.mjs';
import { _ as _sfc_main$4 } from './Checkbox-BX22gW7J.mjs';
import { _ as _sfc_main$5 } from './Textarea-GjRObXeo.mjs';
import { _ as _sfc_main$6 } from './Alert-Dt8CKxt0.mjs';
import { _ as _sfc_main$7 } from './FormField-CCAAD2YO.mjs';
import { _ as _sfc_main$8 } from './Input-il5BObb7.mjs';
import { u as useGuestQuoteStore } from './guestQuote-Cd2LcvdR.mjs';
import { u as useQuoteDraftStore } from './quoteDraft-_7jVZ4Ti.mjs';
import { f as formatItemPrice } from './formatters-C4IR8tCh.mjs';
import '../_/nitro.mjs';
import 'lru-cache';
import '@unocss/core';
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(result) && !unref(error)) {
        _push(`<p class="text-sm text-stone-500 dark:text-stone-400"> Get an estimate for your quote </p>`);
      } else {
        _push(`<!---->`);
      }
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(unref(revealedLines), (line, idx) => {
          ...
          if (line.amount) {
            _push(`<span class="font-medium text-stone-800 dark:text-stone-100 tabular-nums">${ssrInterpolate(line.amount)} ${ssrInterpolate(unref(result).currency)}</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
        ...
        if (!unref(result).can_calculate && unref(result).reason) {
          ...
          _push(`<div class="mt-4 pt-4 border-t border-amber-200/60 dark:border-amber-800/40 flex justify-between font-semibold text-stone-800 dark:text-stone-100"><span>Total</span><span class="tabular-nums">${ssrInterpolate(unref(result).total)} ${ssrInterpolate(unref(result).currency)}</span></div>`);
        } else {
          _push(`<!---->`);
        }
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    async function loadOptions() {
      ...
      loadError.value = "";
      try {
        const opts = await publicApi(API.publicProductOptions(productId));
    ...
    function resetForm() {
      ...
      form.finishings = (i.finishings ?? []).map((f) => ({ finishing_rate: typeof f === "object" && f && "finishing_rate" in f ? f.finishing_rate : Number(f) }));
    }
          ...
          if (unref(loadError)) {
            ...
            if (unref(pricingMode) === "SHEET" && unref(papers).length) {
              ...
              ssrRenderList(unref(papers), (p) => {
                _push2(`<label class="${ssrRenderClass([unref(form).paper === p.id ? "bg-flamingo-50 dark:bg-flamingo-900/20" : "hover:bg-[var(--p-surface-sunken)]", "flex items-center gap-3 rounded-lg px-3 py-2 cursor-pointer transition-colors"])}"><input${ssrIncludeBooleanAttr(ssrLooseEqual(unref(form).paper, p.id)) ? " checked" : ""} type="radio"${ssrRenderAttr("value", p.id)} class="accent-flamingo-500"><span class="text-sm text-[var(--p-text)]">${ssrInterpolate(p.sheet_size)} ${ssrInterpolate(p.gsm)}gsm ${ssrInterpolate(p.paper_type)}</span><span class="ml-auto text-xs text-[var(--p-text-muted)]">KES ${ssrInterpolate(p.selling_price)}</span></label>`);
              });
              _push2(`<!--]--></div></div>`);
            } else {
              _push2(`<!---->`);
            ...
            if (unref(pricingMode) === "LARGE_FORMAT" && unref(materials).length) {
              ...
              ssrRenderList(unref(materials), (m) => {
                _push2(`<label class="${ssrRenderClass([unref(form).material === m.id ? "bg-flamingo-50 dark:bg-flamingo-900/20" : "hover:bg-[var(--p-surface-sunken)]", "flex items-center gap-3 rounded-lg px-3 py-2 cursor-pointer transition-colors"])}"><input${ssrIncludeBooleanAttr(ssrLooseEqual(unref(form).material, m.id)) ? " checked" : ""} type="radio"${ssrRenderAttr("value", m.id)} class="accent-flamingo-500"><span class="text-sm text-[var(--p-text)]">${ssrInterpolate(m.material_type ?? m.name)}</span><span class="ml-auto text-xs text-[var(--p-text-muted)]">KES ${ssrInterpolate(m.selling_price)}/${ssrInterpolate(m.unit)}</span></label>`);
              });
              _push2(`<!--]--></div></div>`);
            } else {
              _push2(`<!---->`);
            ...
            if (unref(finishingRates).length) {
              _push2(`<div><label class="block text-sm font-medium text-[var(--p-text-dim)] mb-1.5">Finishing</label><div class="space-y-1.5 max-h-40 overflow-y-auto rounded-lg border border-[var(--p-border)] p-2"><!--[-->`);
              ssrRenderList(unref(finishingRates), (fr) => {
                ...
                _push2(`<span class="text-sm text-[var(--p-text)] flex-1">${ssrInterpolate(fr.name)}</span><span class="text-xs text-[var(--p-text-muted)]">KES ${ssrInterpolate(fr.price)}</span></label>`);
              });
              _push2(`<!--]--></div></div>`);
            } else {
              _push2(`<!---->`);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-amber-50/80 dark:bg-stone-950" }, _attrs))}><div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10"><div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8"><h1 class="text-2xl sm:text-3xl font-bold text-stone-800 dark:text-stone-100 font-[family-name:var(--font-heading)]"> Your Quote </h1>`);
      _push(ssrRenderComponent(_component_UButton, {
        to: "/shops",
        variant: "outline",
        color: "neutral"
      ...
      if (unref(isAuthenticated) && unref(quoteDraftStore).isLoading && !unref(quoteDraftStore).activeDraft) {
        ...
        ssrRenderList(unref(displayDraft).items, (item) => {
          ...
          if (item.unit_price || item.line_total) {
            _push(`<p class="mt-1 text-xs text-stone-400 dark:text-stone-500"> Unit: ${ssrInterpolate(unref(formatItemPrice)(item.unit_price))} · Total: ${ssrInterpolate(unref(formatItemPrice)(item.line_total))}</p>`);
        ...
        _push(`<h3 class="mt-4 text-lg font-medium text-stone-700 dark:text-stone-300">No items in your quote</h3><p class="mt-2 text-sm text-stone-500 dark:text-stone-400">Browse shops, add products, or request a custom print to get a quote.</p>`);
        _push(ssrRenderComponent(_component_UButton, {
          to: "/shops",
          color: "primary",
          class: "mt-4"
...
//# sourceMappingURL=quote-draft-CUarepBa.mjs.map
