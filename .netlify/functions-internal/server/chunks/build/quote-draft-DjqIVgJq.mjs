...
import { g as useRoute, c as useAuthStore$1, h as useAnalyticsTracking, i as useToast, a as _sfc_main$f, d as _sfc_main$9, n as navigateTo, j as usePublicApiNoAuth, A as API } from './server.mjs';
...
import { f as formatCurrency, a as formatDateTime, b as formatKES } from './formatters-C39vX7Ji.mjs';
import { previewPrice } from './quoteDraft-BhhNn4oA.mjs';
import { s as setInterval } from './interval-DiSDr_Za.mjs';
import { _ as _sfc_main$4 } from './Checkbox-Cm9Vi01u.mjs';
import { _ as _sfc_main$5 } from './Textarea-CeohXmxE.mjs';
import { c as calculateGalleryProductPrice } from './gallery-GtLUt84g.mjs';
import { useDebounceFn } from '@vueuse/core';
import { _ as _sfc_main$6 } from './Alert-CRX5veBl.mjs';
import { _ as _sfc_main$7 } from './FormField-BD4oNIMk.mjs';
import { _ as _sfc_main$8 } from './Input-C16lzpZD.mjs';
import { u as useGuestQuoteStore } from './guestQuote-mHa2_mLj.mjs';
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(isAuthenticated) && unref(quoteDraftStore).isLoading && !unref(quoteDraftStore).activeDraft) {
        ...
        ssrRenderList(unref(filteredItems), (item) => {
          ...
          _push(`<!--]--></div></div><div class="mt-6 flex flex-col gap-4 border-t border-[var(--p-border)] pt-4 xl:flex-row xl:items-center xl:justify-between"><div><p class="text-sm font-medium text-[var(--p-text-muted)]">${ssrInterpolate(itemFootnote(item))}</p>`);
          if (itemAddedAt(item)) {
            _push(`<p class="mt-1 text-xs text-[var(--p-text-muted)]"> Added ${ssrInterpolate(itemAddedAt(item))}</p>`);
          } else {
            _push(`<!---->`);
          }
          _push(`<p class="mt-1 text-sm text-[var(--p-text-muted)]">${ssrInterpolate(item.unit_price || item.line_total ? pricingLine(item) : guestPricingLine)}</p></div><div class="flex flex-wrap items-center gap-3"><div class="flex items-center gap-1 rounded-lg border border-[var(--p-border)] bg-[var(--p-surface-container-low)]">`);
          _push(ssrRenderComponent(_component_UButton, {
...
//# sourceMappingURL=quote-draft-DjqIVgJq.mjs.map
