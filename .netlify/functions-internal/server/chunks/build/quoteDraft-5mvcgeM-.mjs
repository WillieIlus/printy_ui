...
import { A as API, p as parseApiError, m as useNuxtApp } from './server.mjs';
import 'vue';
import '../_/nitro.mjs';
import 'lru-cache';
import '@unocss/core';
...
async function requestQuoteItem(draftId, itemId, api) {
  return await resolveApi(api).post(API.quoteDraftItemRequestQuote(draftId, itemId), {});
}
async function listQuoteDraftFiles(api, scope = "draft") {
  const params = scope === "dashboard" ? { scope: "dashboard" } : void 0;
  return await resolveApi(api).get(API.quoteDraftFiles(), params);
}
...
export { addItem, downloadQuoteDraftFilePdf, downloadQuoteDraftPdf, getActiveDraft, getQuoteDraftFile, listQuoteDraftFiles, listQuoteRequests, previewPrice, previewQuoteDraftFileWhatsapp, removeItem, requestQuote, requestQuoteItem, tweakAndAdd, updateItem, updateQuoteDraftFile };
//# sourceMappingURL=quoteDraft-5mvcgeM-.mjs.map
