import { previewPrice, getActiveDraft, requestQuote, removeItem, updateItem, addItem } from './quoteDraft-BhhNn4oA.mjs';
import { y as safeLogError } from './server.mjs';
import { ref } from 'vue';
...
const useQuoteDraftStore = defineStore("quoteDraft", () => {
  ...
  async function addTweakedProductToQuote(shopSlug, payload) {
    const draft = await ensureDraftForShop(shopSlug);
    if (!draft) return null;
    const { tweakAndAdd } = await import('./quoteDraft-BhhNn4oA.mjs');
...
//# sourceMappingURL=quoteDraft-CE4LeCAM.mjs.map
