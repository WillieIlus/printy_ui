...
import { m as useNuxtApp, A as API, p as parseApiError, s as safeLogError, J as extractApiFeedback } from './server.mjs';
...
const useShopStore = defineStore("shop", () => {
  ...
  const selectedShopSlug = useStorage(
    "active-shop-slug",
    null,
    getBrowserStorage()
  );
  ...
  function buildShopPatchPayload(data) {
    ...
    if (data.google_place_id !== void 0) payload.google_place_id = normalizeOptionalText(data.google_place_id);
  ...
  async function fetchMyShops() {
    ...
    try {
      ...
      const hasSelectedShop = selectedShopSlug.value ? list.some((shop) => shop.slug === selectedShopSlug.value) : false;
      if (!hasSelectedShop) {
  ...
  function selectActiveShop(slug) {
  ...
  async function ensureActiveShop(slug) {
...
//# sourceMappingURL=shop-DqJLBw0V.mjs.map
