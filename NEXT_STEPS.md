...
- **Environment:** `.env` uses `NUXT_PUBLIC_API_BASE` (API base). Local value: `http://localhost:8000/api`.
...
- **CORS:** Ensure the API allows requests from your frontend origin (e.g. `https://printy.ke`).
...
3. **Hosting:** Deploy the output (e.g. to Netlify). Set `NUXT_PUBLIC_API_BASE=https://api.printy.ke/api` in Site settings so production uses the correct backend.
