<template>
  <div class="col-span-12 flex min-w-0 flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
    <div class="min-w-0 flex-1 space-y-1">
      <h1 class="break-words text-2xl font-semibold text-[var(--p-text)]">
        {{ shopName }}
      </h1>
      <p class="break-words text-sm text-[var(--p-text-muted)]">
      ...
      <div v-if="publicUrl" class="flex min-w-0 flex-wrap items-center gap-2 pt-2">
        <NuxtLink
          :to="publicUrl"
          target="_blank"
          class="inline-flex shrink-0 items-center gap-1.5 text-sm font-medium text-flamingo-600 hover:underline dark:text-flamingo-400"
        >
