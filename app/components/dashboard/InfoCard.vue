<template>
  ...
  >
    <div class="flex items-start gap-3">
      <div class="mt-0.5 flex h-10 w-10 items-center justify-center rounded-2xl border border-white/10 bg-white/10">
        <UIcon :name="icon" class="h-5 w-5" />
      </div>
      <div class="min-w-0 space-y-1">
        <p class="text-sm font-semibold text-white">{{ title }}</p>
        <p v-if="description" class="text-sm leading-6 text-slate-200/90">
          {{ description }}
        </p>
