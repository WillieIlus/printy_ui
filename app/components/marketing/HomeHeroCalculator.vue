<template>
  <div>
    ...
    <section class="relative overflow-hidden bg-[#101828]">
      ...
      <div class="relative mx-auto max-w-[1500px] px-4 sm:px-6" :class="props.embedded ? 'py-8' : 'py-8 lg:py-10'">
        <div class="grid items-center" :class="props.embedded ? 'gap-0' : 'gap-7'">
          <div v-if="!props.embedded" class="mx-auto max-w-5xl text-center">
            <div class="mb-4 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/8 px-4 py-1.5 backdrop-blur-sm">
            ...
            <h1 class="mb-3 text-3xl font-extrabold leading-tight tracking-tight md:text-5xl">
              <span class="text-white">Get real</span>
              <span class="text-[#f97316]">print prices</span>
              <span class="text-white">before you call a printer</span>
            </h1>

            <p class="mx-auto mb-5 max-w-3xl text-base leading-relaxed text-slate-200 md:text-lg">
              Configure the job, see a live estimated market range, then choose who manages the job after continuing.
            </p>

            <div class="flex flex-wrap justify-center gap-3">
              <NuxtLink to="/auth/register?role=client&next=%2Fdashboard%3FpendingQuote%3D1&pendingQuote=1" class="inline-flex items-center gap-2 rounded-xl bg-[#e13515] px-7 py-3.5 text-base font-semibold text-white shadow-lg transition-colors hover:bg-[#b82c10]" @click.prevent="continueWithEstimate('register')">
                Get instant print price
                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                  <path d="M5 12h14M12 5l7 7-7 7" />
                </svg>
              </NuxtLink>
              <NuxtLink to="/about" class="inline-flex items-center gap-2 rounded-xl border border-[#475467] px-7 py-3.5 text-base font-semibold text-[#d0d5dd] transition-colors hover:border-[#667085] hover:text-white">
                See how Printy works
              </NuxtLink>
            </div>
          </div>

          <div id="live-estimate" class="relative mx-auto w-full max-w-[1320px]">
            ...
            <div class="overflow-hidden rounded-2xl border border-[#e4e7ec] bg-white shadow-2xl">
              <div ref="calculatorTopSentinel" aria-hidden="true" class="h-px w-full" />
              ...
              <div class="bg-[#f9fafb] border-b border-[#e4e7ec] px-6 py-4 flex items-center justify-between gap-4">
                <div v-show="artworkUploaded" class="transition-all duration-300 ease-out">
                  <p class="text-xs font-semibold uppercase tracking-widest text-[#667085] mb-0.5">Estimated Market Range</p>
              ...
              <div class="grid gap-6 px-4 py-5 text-[#101828] sm:px-6 lg:grid-cols-12">
                <input ref="artworkInput" type="file" accept=".pdf,.ai,.psd,.png,.jpg,.jpeg,.webp" class="hidden" @change="handleArtworkSelect">

                <div class="contents lg:order-1 lg:col-span-7 lg:block lg:space-y-5">
                <div class="hidden rounded-2xl border border-[#e4e7ec] bg-[#f9fafb] px-5 py-4 sm:block">
                  <p class="text-[14px] font-extrabold text-[#101828] sm:text-[15px]">Start with what you need printed</p>
                  <p class="mt-1 text-[12px] text-[#475467]">Choose a product, quantity, and finish. Printy will show a live estimated market range before you continue.</p>
                  <div class="mt-3 hidden flex-wrap gap-2 sm:flex">
                    <span class="rounded-full border border-[#e4e7ec] bg-white px-2.5 py-1 text-[11px] font-semibold text-[#667085]">Estimated market range in KES</span>
                    <span class="rounded-full border border-[#e4e7ec] bg-white px-2.5 py-1 text-[11px] font-semibold text-[#667085]">Upload artwork when ready</span>
                    <span class="rounded-full border border-[#e4e7ec] bg-white px-2.5 py-1 text-[11px] font-semibold text-[#667085]">Adjust before confirming</span>
                  </div>
                </div>
                <div class="order-2 lg:order-none">
                  ...
                  <div class="flex gap-2 overflow-x-auto pb-1">
                    <button
                      ...
                      :key="product.key"
                      type="button"
                      class="relative flex h-[64px] min-w-[64px] flex-col items-center justify-center rounded-xl border-2 px-2 py-2 text-center transition-all sm:h-[86px] sm:min-w-[132px] sm:px-3"
                ...
                <div class="order-3 rounded-3xl border border-[#e4e7ec] bg-[#f9fafb] px-5 py-4 lg:order-none">
                  ...
                  <div v-if="unitPriceText" class="mt-3 border-t border-[#e4e7ec] pt-3">
                    <p class="text-[12px] font-bold text-[#101828]">{{ unitPriceText }}</p>
                    <p v-if="bulkValueHintText" class="mt-1 text-[10.5px] text-[#98a2b3]">{{ bulkValueHintText }}</p>
                  </div>
                </div>

                <div class="order-4 flex items-center justify-between gap-3 rounded-2xl border border-[#e4e7ec] bg-[#f9fafb] px-4 py-3 lg:order-none">
                  <div>
                ...
                <div v-show="finishingOptionsOpen" class="order-5 grid gap-4 md:grid-cols-2 lg:order-none">
                ...
                <div class="order-6 grid gap-4 md:grid-cols-2 lg:order-none">
                  <div>
                ...
                <div class="order-7 overflow-hidden rounded-3xl border-2 border-dashed transition-colors lg:order-none" :class="artworkDropzoneClass">
                  <button
                    type="button"
                ...
                <div v-show="artworkUploaded && finishingOptionsOpen" class="order-8 rounded-3xl border border-[#e4e7ec] bg-[#f9fafb] px-5 py-4 transition-all duration-300 ease-out lg:order-none">
                ...
                <div v-show="artworkUploaded" class="order-9 rounded-3xl border border-[#e4e7ec] bg-[#f9fafb] px-5 py-4 transition-all duration-300 ease-out lg:order-none">
                  ...
                  <div class="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <span class="text-[10.5px] text-[#98a2b3]">Sheet finishing follows the selected sides where pricing supports it.</span>
                  </div>
                </div>

                </div>

                <div class="contents lg:order-2 lg:col-span-5 lg:block lg:sticky lg:top-6 lg:self-start lg:space-y-4">
                <div class="order-1 rounded-2xl border border-[#e4e7ec] bg-white p-4 lg:order-none">
                  <div class="relative w-full overflow-hidden rounded-2xl border border-[#e4e7ec] bg-[#f9fafb]">
                    <img :src="productPreviewImage" :alt="productLabel" class="h-56 w-full object-cover sm:h-64 lg:h-72" loading="lazy">
                    <Transition name="fade">
                      <div v-if="unitPriceText" class="absolute bottom-3 right-3 rounded-lg bg-[#101828]/90 px-2.5 py-1.5 text-[12px] font-bold text-white shadow-lg backdrop-blur-sm">
                        {{ unitPriceText }}
                      </div>
                    </Transition>
                  </div>
                  <div class="mt-4">
                    <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-[#e13515]">Selected product</p>
                    <p class="mt-1 text-[16px] font-extrabold text-[#101828] sm:text-[18px]">{{ productLabel }}</p>
                    <p class="mt-1 text-[12px] text-[#475467]">{{ productPreviewTagline }}</p>
                    <p class="mt-2 text-[11px] font-semibold text-[#667085]">{{ productPreviewSpecLine }}</p>
                  </div>
                </div>

                <div class="order-10 rounded-3xl bg-[#101828] px-5 py-4 lg:order-none">
              ...
              <div class="order-11 rounded-3xl bg-[#1d2939] px-5 py-4 lg:order-none">
                <div class="mb-3 flex items-center gap-2">
              ...
              <div class="order-12 rounded-xl border-2 border-[#e13515] px-5 py-4 lg:order-none">
                ...
                <div v-if="preview.turnaround_tiers" class="mt-3 space-y-1">
                  <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">
                    Choose your turnaround
                  </p>
                  <div class="space-y-1">
                    <button
                      v-for="(tier, key) in preview.turnaround_tiers"
                      :key="key"
                      type="button"
                      class="flex w-full items-center justify-between rounded-xl border px-3 py-2 text-sm transition-colors"
                      :class="selectedUrgency === String(key)
                        ? 'border-[#e13515] bg-[#fef3f2] text-[#101828]'
                        : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300'"
                      @click="selectedUrgency = String(key)"
                    >
                      <span>{{ tier.label }}</span>
                      <span class="font-semibold">KES {{ tier.total }}</span>
                    </button>
                  </div>
                </div>
              </div>

              <div class="order-13 px-5 py-2 lg:order-none">
                ...
                <div class="mt-4 flex flex-col gap-3 sm:flex-row">
                  <template v-if="auth.canAccessClientDashboard">
                    ...
                    >
                      {{ quoteRequestLoading ? 'Saving draft...' : 'Continue with this quote' }}
                    </button>
                  </template>
                  ...
                  <template v-else>
                    <NuxtLink to="/auth/register?role=client&next=%2Fdashboard%3FpendingQuote%3D1&pendingQuote=1" class="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#e13515] px-4 py-3 text-center text-[14px] font-extrabold text-white transition-colors hover:bg-[#b82c10]" @click.prevent="continueWithEstimate('register')">
                      Continue with this quote
                    </NuxtLink>
                    <NuxtLink to="/auth/login?next=%2Fdashboard%3FpendingQuote%3D1&pendingQuote=1" class="flex w-full items-center justify-center gap-2 rounded-2xl border border-[#fda497] bg-white px-4 py-3 text-center text-[14px] font-bold text-[#c4320a] transition-colors hover:bg-[#fff1ee]" @click.prevent="continueWithEstimate('login')">
              ...
              <div class="order-14 flex flex-wrap items-center gap-3 rounded-2xl border border-[#e4e7ec] bg-[#f9fafb] px-4 py-3 lg:order-none">
                <span class="text-[11px] font-semibold uppercase tracking-widest text-[#667085]">Estimated range</span>
                ...
                <span v-else class="text-[12px] text-[#667085]">Estimated market range in KES based on your selected product, quantity, paper, and finish.</span>
              </div>
              <div class="order-15 flex items-start gap-3 rounded-lg border border-[#fde8e2] bg-[#fffbf9] px-4 py-3 lg:order-none">
                ...
                <p class="text-xs text-[#667085] leading-relaxed">
                  You can adjust before confirming. Final pricing may depend on <span class="font-semibold text-[#344054]">artwork, delivery, turnaround,</span> and
                  <span class="font-semibold text-[#344054]">manager confirmation.</span>
                </p>
              </div>

              <div class="order-16 px-6 pb-6 lg:order-none">
                <template v-if="props.embedded && auth.canAccessClientDashboard">
                  <div class="mb-4 rounded-2xl border border-[#e4e7ec] bg-[#f8fafc] p-4">
                    ...
                    <p class="mt-2 text-sm leading-6 text-[#475467]">Save this estimate, then choose who manages the job on the next screen. Production partners stay hidden until dispatch.</p>
                  </div>
    ...
    <section v-if="false" id="how-it-works" class="bg-white py-20">
    ...
    <section v-if="false" id="who-it-helps" class="bg-[#f9fafb] py-24 border-t border-[#e4e7ec]">
      <div class="max-w-7xl mx-auto px-6">
    ...
    <section v-if="false" id="why-trust" class="bg-white py-24 border-t border-[#e4e7ec]">
      <div class="max-w-7xl mx-auto px-6">
    ...
    <section v-if="false" class="bg-white py-24 border-t border-[#e4e7ec]">
      <div class="max-w-7xl mx-auto px-6">
        <div class="text-center mb-16">
    ...
    <Transition name="sticky-slide">
      <div
        v-if="showMobileStickyBar && estimateDisplayText"
        class="fixed inset-x-0 bottom-0 z-40 border-t border-[#e4e7ec] bg-white shadow-[0_-4px_20px_rgba(16,24,40,0.08)] md:hidden"
      >
        <div class="flex items-center gap-3 px-4 py-3">
          <div class="min-w-0 flex-1">
            <p class="text-[10px] font-bold uppercase tracking-[0.14em] text-[#667085]">Live estimate</p>
            <p class="truncate text-[14px] font-extrabold text-[#101828]">{{ estimateDisplayText }}</p>
          </div>
          <template v-if="auth.canAccessClientDashboard">
            <button
              type="button"
              class="shrink-0 rounded-xl bg-[#e13515] px-4 py-2.5 text-[13px] font-extrabold text-white transition-colors hover:bg-[#b82c10] disabled:cursor-not-allowed disabled:opacity-60"
              :disabled="quoteRequestLoading || !hasMinimumQuoteInputs"
              @click="submitQuoteRequest"
            >
              Continue
          ...
          <template v-else-if="auth.isAuthenticated">
            <NuxtLink
              :to="auth.homeRoute"
              class="shrink-0 rounded-xl bg-[#e13515] px-4 py-2.5 text-[13px] font-extrabold text-white"
            >
              Open
            </NuxtLink>
          </template>
          <template v-else>
            <NuxtLink
              ...
              class="shrink-0 rounded-xl bg-[#e13515] px-4 py-2.5 text-[13px] font-extrabold text-white"
              @click.prevent="continueWithEstimate('register')"
            >
              Continue
            </NuxtLink>
          </template>
...
const preview = ref<Record<string, any>>({
  summary: 'Adjust the options above to see a live estimated market range before continuing.',
...
const calculatorTopSentinel = ref<HTMLElement | null>(null)
...
const productImageMap: Record<string, string> = {
  business_card: '/products/business-card.png',
  flyer: '/products/flyer.png',
  label_sticker: '/products/label-sticker.png',
  letterhead: '/products/letterhead.png',
  booklet: '/products/booklet.png',
  large_format: '/products/large-format.png',
}
const productImageFallback = '/products/business-card.png'
const productPreviewTaglineMap: Record<string, string> = {
  business_card: 'Make a strong first impression in every meeting.',
  flyer: 'Hand out, post up, get noticed.',
  label_sticker: 'Stick your brand on every product.',
  letterhead: 'Look official on every page you send.',
  booklet: 'Tell your full story in one organized print.',
  large_format: 'Be seen across the room or across the street.',
}
...
function savePendingEstimate(source: 'homepage' | 'dashboard' = 'homepage') {
  ...
  pendingClientQuote.save({
    ...
    urgency_type: selectedUrgency.value,
...
function saveCalculatorHint() {
  const estimatedTotal = minPriceValue.value || maxPriceValue.value || parseMoneyValue(preview.value.total)
  const quantity = Number(form.value.quantity || 0)
  calculatorHint.setHint({
    productType: String(form.value.product_type || '').trim() || null,
    quantity: quantity > 0 ? quantity : null,
    turnaround: selectedUrgency.value === 'express' || selectedUrgency.value === 'same_day' ? selectedUrgency.value : 'standard',
    estimatedUnitPrice: estimatedTotal > 0 && quantity > 0 ? estimatedTotal / quantity : null,
    estimatedTotal: estimatedTotal > 0 ? estimatedTotal : null,
    currency: String(preview.value.currency || 'KES'),
  })
}
...
function applyPendingEstimate() {
  ...
  selectedUrgency.value = pending.urgency_type || selectedUrgency.value
...
function applyDetectedSpecs() {
  ...
  artworkIntent.value = applied ? 'detected' : 'manual'
    ...
    ? 'Detected local values applied to the calculator. The estimate will refresh with those details.'
    : 'Suggestion saved, but the current calculator options do not expose a matching flyer or A4 option.'
}
...
async function refreshPreview() {
  ...
  try {
    ...
    preview.value = {
      summary: 'Printy could not refresh this estimate right now. Try adjusting an option or check back in a moment.',
      price_mode: 'error',
    }
    previewErrorMessage.value = 'We could not refresh the live estimate. Please try again in a moment.'
  } finally {
    loading.value = false
  }
}
...
onMounted(async () => {
  ...
  try {
    ...
    await refreshPreview()
      ...
      preview.value = {
        summary: 'Printy could not load pricing options right now. Please refresh the page in a moment.',
      price_mode: 'error',
    }
    previewErrorMessage.value = 'The calculator could not load pricing options.'
  } finally {
    loading.value = false
  }

  if (import.meta.client && calculatorTopSentinel.value) {
    ...
    stickyObserver.observe(calculatorTopSentinel.value)
  }
})
...
const productPreviewImage = computed(() => productImageMap[form.value.product_type] || productImageFallback)
const productPreviewTagline = computed(() => productPreviewTaglineMap[form.value.product_type] || 'Premium print, transparently priced.')
const productPreviewSpecLine = computed(() => {
  const finish = laminationLabel.value === 'No lamination' ? 'No finish' : laminationLabel.value
  return `${quantityText.value} pcs · ${sizeLabel.value} · ${finish}`
})
...
const minPriceValue = computed(() => parseMoneyValue(preview.value.estimate_min ?? preview.value.min_price ?? preview.value.total))
const maxPriceValue = computed(() => parseMoneyValue(preview.value.estimate_max ?? preview.value.max_price ?? preview.value.total))
const unitPriceText = computed(() => {
  ...
  const perPiece = midpoint / quantity
  const formatted = formatPerPiece(perPiece)
  return formatted ? `≈ ${formatted} per piece` : ''
})
const bulkValueHintText = computed(() => {
  if (!unitPriceText.value) {
    return ''
  }
  const quantity = quantityValue.value
  if (quantity < 250) {
    return 'Better value at 500+'
  }
  if (quantity < 500) {
    return 'Better value at 1,000+'
  }
  if (quantity < 1000) {
    return 'Better value at 5,000+'
  }
  return ''
})
const estimateDisplayText = computed(() => {
  const explicit = String(preview.value.display_price_text || '').trim()
  if (explicit && !/\b0(?:\.00)?\b/.test(explicit.replace(/,/g, ''))) {
    return explicit
  }
  const min = minPriceValue.value
  const max = maxPriceValue.value
  if (min > 0 && max > 0) {
    if (min === max) {
      return `From ${formatKes(min)} to ${formatKes(max * 1.1)}`
    }
    return `From ${formatKes(min)} to ${formatKes(max)}`
  }
...
const estimateNote = computed(() => preview.value.summary || 'Final pricing may depend on artwork, delivery, turnaround, and manager confirmation.')
const exactQuoteCtaText = computed(() => 'Continue with this quote')
const exactQuoteSupportText = computed(() => auth.canAccessClientDashboard
  ? 'Save these specs and choose who manages the job after continuing.'
  : 'We’ll keep these specs while you sign in or create a client account.')
...
const workflowSteps = [
  {
    ...
    detail: 'Use the live controls above to change quantity, size, stock, sides, and finish. Printy uses those exact inputs so the estimate is grounded in real pricing logic.',
    cta: 'signup',
  ...
  },
  {
    ...
    detail: 'The estimate card shows live KES range data, sheet counts, and a plain-language summary so buyers can judge cost before handing over artwork or chasing anyone on WhatsApp.',
...
function formatPerPiece(value: number) {
  if (!Number.isFinite(value) || value <= 0) {
    return ''
  }
  if (value >= 10) {
