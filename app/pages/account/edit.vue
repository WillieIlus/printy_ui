    ...
    <section class="rounded-3xl border border-[var(--p-border)] bg-[var(--p-surface)] p-6 shadow-sm">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h1 class="mt-2 text-2xl font-semibold text-[var(--p-text)]">Edit profile</h1>
          <p class="mt-2 text-sm text-[var(--p-text-muted)]">
            Update your profile details, public links, and avatar with the same live account API used in the customer workspace.
          </p>
        </div>
        <UButton variant="soft" @click="goBack">Back to Account</UButton>
      </div>

      <div class="mt-6 grid gap-6 xl:grid-cols-[1.2fr_0.85fr]">
        <DashboardFormSection title="Profile Details" description="Name, language, phone, address, bio, avatar, and social links are supported here through the live account API.">
