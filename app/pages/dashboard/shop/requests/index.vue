              ...
              <div class="space-y-5">
                ...
                <div class="flex flex-wrap gap-2 border-t border-white/10 pt-5">
                  <BaseButton
                    variant="primary"
                    size="sm"
                    :to="`/dashboard/shop/requests/${card.id}`"
                  >
                    Open request
                  </BaseButton>
                  <BaseButton
                    v-if="card.latestQuoteId"
                    variant="outline"
                    size="sm"
                    :to="`/dashboard/shop/requests/${card.id}/quote/${card.latestQuoteId}`"
                  >
                    View quote
                  </BaseButton>
                  <BaseButton variant="outline" size="sm" @click="openAction(card.id, 'quote')">Respond with quote</BaseButton>
                  <BaseButton variant="outline" size="sm" @click="openAction(card.id, 'question')">Ask buyer a question</BaseButton>
                  <BaseButton variant="ghost" size="sm" @click="openAction(card.id, 'reject')">Mark cannot fulfill</BaseButton>
                ...
                >
                  ...
                  <form class="space-y-4" @submit.prevent="submitAction(card)">
                    <div v-if="activeAction.kind === 'quote'" class="space-y-4">
...
function buildRequestCard(raw: IncomingRequestRecord): RequestCard {
  ...
  const rawItems = Array.isArray(raw.items) ? raw.items as IncomingRequestRecord[] : []
  const firstItem = rawItems[0] ?? {}

  const jobType = titleCase(
    stringValue(calculatorInputs.product_type)
    || stringValue(firstItem.product_name)
    || stringValue(firstItem.title)
    || 'Custom print job'
  )
  const quantity = numberValue(calculatorInputs.quantity) ?? numberValue(firstItem.quantity)
  const sizeLabel = stringValue(calculatorInputs.finished_size)
    ...
    || (firstItem.chosen_width_mm && firstItem.chosen_height_mm
      ? `${firstItem.chosen_width_mm}x${firstItem.chosen_height_mm}mm`
      : 'Size not specified')

  const itemFinishings = rawItems.flatMap(item =>
    Array.isArray((item as IncomingRequestRecord).finishings)
      ? ((item as IncomingRequestRecord).finishings as IncomingRequestRecord[]).map(f => titleCase(stringValue(f.finishing_rate_name) || stringValue(f.finishing_rate)))
      : []
  ).filter(Boolean)
  ...
  const briefParts = [
    ...
    sizeLabel,
    stringValue(calculatorInputs.paper_stock) || stringValue(firstItem.paper_label)
      ? titleCase(stringValue(calculatorInputs.paper_stock) || stringValue(firstItem.paper_label))
      : '',
  ].filter(Boolean)

  return {
    id: Number(raw.id),
    latestQuoteId: numberValue(latestSentQuote?.id),
    group,
    ...
    finishings,
    needsConfirmation,
    notes: stringValue(requestDetails.notes) || stringValue(raw.notes),
...
async function shareBrief(card: RequestCard) {
  ...
  try {
    ...
    if (navigator.share) {
      ...
      toast.info('Brief copied', 'The buyer brief is ready to paste or forward.', { context: 'dashboard' })
    } else {
      throw new Error('Sharing is not available in this browser.')
    }
  } catch (error) {
    if (error instanceof Error && error.name === 'AbortError') return
    toast.error('Share brief failed', error instanceof Error ? error.message : 'Could not prepare the buyer brief.', { context: 'dashboard' })
  } finally {
...
async function downloadBriefPdf(card: RequestCard) {
  ...
  try {
    ...
    toast.error('Download failed', error instanceof Error ? error.message : 'Could not download the request PDF.', { context: 'dashboard' })
  } finally {
    briefBusy.value = false
  }
}

function openBuyerWhatsApp(card: RequestCard) {
  if (!card.whatsappUrl) {
    toast.whatsappMissingNumber()
    return
  }

  try {
    toast.whatsappReady(card.buyerName || 'Buyer')
    window.open(card.whatsappUrl, '_blank', 'noopener')
  } catch (error) {
    toast.whatsappFailed(error)
  }
}
...
async function loadRequests() {
  ...
  if (!shopSlug) {
    ...
    cards.value = []
    toast.dashboardLoadFailed(loadError.value)
    return
  }
...
function openAction(cardId: number, kind: ActionKind) {
  ...
  actionForm.availabilityStatus = 'Ready to produce'
  actionForm.alternativeSuggestion = ''
}
