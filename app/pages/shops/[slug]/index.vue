    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <template v-else-if="catalog?.shop">
        <section class="overflow-hidden rounded-[2rem] border border-sky-200/70 bg-white/80 shadow-[0_32px_90px_-52px_rgba(8,24,52,0.42)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/90">
          <div class="border-b border-sky-100/80 bg-[linear-gradient(135deg,rgba(8,24,52,0.98),rgba(13,37,73,0.94) 48%,rgba(16,64,116,0.92) 100%)] px-6 py-6 text-white dark:border-slate-800 sm:px-8">
            <div class="grid gap-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(20rem,0.95fr)]">
              <div class="min-w-0">
                <div class="mb-4 flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-sky-100/70">
                ...
                <div class="flex flex-wrap items-start gap-3">
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-sky-100/78">
                ...
                <div class="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  <span class="inline-flex items-center gap-2 rounded-full border border-orange-300/25 bg-orange-500/18 px-3 py-1 text-xs font-medium text-orange-50">
                ...
                <div v-if="catalog.shop.description" class="mt-5 rounded-2xl border border-white/10 bg-white/8 p-4 backdrop-blur-sm">
                  <p class="mb-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-sky-100/68">
                    Shop overview
                  </p>
                  <EditorRichTextDisplay :html="catalog.shop.description" class="text-sm leading-6 text-sky-50/88" />
                </div>
              </div>

              <div class="space-y-4">
                <div class="rounded-[1.75rem] border border-white/12 bg-white/10 p-5 shadow-[0_24px_50px_-36px_rgba(2,6,23,0.96)] backdrop-blur-md">
                  <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-sky-100/68">
                    Quick actions
                  </p>
                ...
                <div class="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
                  <div
                    ...
                    :key="stat.label"
                    class="rounded-[1.5rem] border border-white/12 bg-slate-950/26 p-4 backdrop-blur-sm"
                  >
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-100/68">{{ stat.label }}</p>
          ...
          <div class="px-6 py-6 sm:px-8">
            <div class="flex flex-wrap items-end justify-between gap-4">
              <div>
                <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-sky-700/80 dark:text-sky-300/80">
                  Catalog
                </p>
              ...
              <div class="inline-flex items-center gap-2 rounded-full border border-sky-200 bg-sky-50 px-4 py-2 text-xs font-medium text-sky-800 dark:border-sky-800 dark:bg-sky-950/40 dark:text-sky-200">
            ...
            <div v-if="catalog.products.length" class="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
              <article
                ...
                :key="product.id"
                class="group overflow-hidden rounded-[1.8rem] border border-sky-100 bg-white shadow-[0_24px_60px_-40px_rgba(15,23,42,0.28)] transition-all hover:-translate-y-1.5 hover:border-orange-200 hover:shadow-[0_30px_70px_-42px_rgba(249,115,22,0.34)] dark:border-slate-800 dark:bg-slate-900"
              >
                ...
                <div class="space-y-4 p-5">
                  ...
                  <div class="grid grid-cols-1 gap-2 text-xs text-slate-600 dark:text-slate-300">
                    <div v-if="product.final_size" class="flex items-center gap-2 rounded-2xl border border-sky-100 bg-sky-50/70 px-3 py-2 dark:border-slate-800 dark:bg-slate-950/60">
                      <UIcon name="i-lucide-ruler" class="h-3.5 w-3.5 shrink-0" />
                      <span>{{ product.final_size }}</span>
                    </div>
                    <div v-if="product.imposition_summary" class="flex items-center gap-2 rounded-2xl border border-sky-100 bg-sky-50/70 px-3 py-2 dark:border-slate-800 dark:bg-slate-950/60">
                      <UIcon name="i-lucide-grid-2x2" class="h-3.5 w-3.5 shrink-0" />
                      <span>Fits on {{ product.imposition_summary }}</span>
                    </div>
                    <div v-if="product.min_quantity" class="flex items-center gap-2 rounded-2xl border border-sky-100 bg-sky-50/70 px-3 py-2 dark:border-slate-800 dark:bg-slate-950/60">
                  ...
                  <div v-if="product.finishing_summary?.length" class="flex flex-wrap gap-2">
                    <span
                      ...
                      :key="finish"
                      class="inline-flex items-center rounded-full border border-sky-100 bg-white px-3 py-1 text-[11px] font-medium text-slate-700 dark:border-slate-700 dark:bg-slate-950/70 dark:text-slate-200"
                    >
                      {{ finish }}
                    </span>
                  </div>
            ...
            <div v-else class="mt-6 rounded-[1.8rem] border border-dashed border-sky-200 bg-sky-50/70 p-12 text-center dark:border-slate-700 dark:bg-slate-950/60">
              ...
              <h3 class="mt-4 text-lg font-semibold text-slate-950 dark:text-slate-100">No products yet</h3>
              <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">This shop has not published products to its public catalog yet.</p>
            </div>
          </div>
        </section>

        <section v-if="pricingStore.rateCard" class="mt-8">
          <div class="rounded-[1.8rem] border border-sky-100 bg-white/85 p-6 shadow-[0_24px_60px_-42px_rgba(15,23,42,0.26)] dark:border-slate-800 dark:bg-slate-900/90">
      ...
      <div v-else class="rounded-[1.8rem] border border-sky-100 bg-white/90 p-12 text-center shadow-[0_24px_60px_-42px_rgba(15,23,42,0.26)] dark:border-slate-800 dark:bg-slate-900/90">
        ...
        <h3 class="mt-4 text-lg font-semibold text-slate-950 dark:text-slate-100">Shop not found</h3>
        <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">The shop you are looking for does not exist or is inactive.</p>
