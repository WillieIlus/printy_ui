...
export async function updateManagerProofApproval(id: number | string, action: 'approve' | 'reject', note = '') {
  const { api } = useApi()
  return api<Record<string, any>>(API.jobs.managerJobProofApproval(id), {
    method: 'POST',
    body: { action, note },
  })
}
