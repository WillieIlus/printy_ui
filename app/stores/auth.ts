...
import type { AuthTokens, AuthUser, DashboardRole, LoginPayload, RegisterPayload, RegisterResponse, ResetPasswordPayload } from '~/shared/types'
import { dashboardRoleFromPath, dashboardRouteForRole, isRoleAccessible, normalizeDashboardRole, resolveAccessibleRoles, resolveDashboardRole } from '~/shared/workspace'
...
function getStoredActiveRole(): DashboardRole | null {
  return normalizeDashboardRole(getRawCookie('printy_active_role'))
}

function setStoredActiveRole(role: DashboardRole | null) {
  setRawCookie('printy_active_role', role, role ? THIRTY_DAYS_IN_SECONDS : undefined)
}
...
export const useAuthStore = defineStore('auth', {
  state: () => ({
    ...
    activeRole: null as DashboardRole | null,
    initialized: false,
    isLoading: false,
  }),
  getters: {
    ...
    dashboardRole: (state) => resolveDashboardRole(state.user, state.activeRole),
    ...
    homeRoute: (state) => dashboardRouteForRole(resolveDashboardRole(state.user, state.activeRole)),
  },
  actions: {
    clearSession() {
      setTokens(null)
    ...
    hydrateActiveRole() {
      if (!this.activeRole) {
        this.activeRole = getStoredActiveRole()
      }
    },
    setActiveRole(role: DashboardRole | string | null) {
      ...
      if (this.user && !isRoleAccessible(this.user, normalized)) {
        return false
      }
      this.activeRole = normalized
      setStoredActiveRole(normalized)
      return true
    },
    syncActiveRoleFromUser(user: AuthUser | null) {
      if (!user) {
        return
      }
      const selected = this.activeRole && isRoleAccessible(user, this.activeRole)
        ? this.activeRole
        : normalizeDashboardRole(user.active_role || user.active_dashboard_role || user.dashboard_role || user.primary_role || user.role)
      this.activeRole = isRoleAccessible(user, selected) ? selected : resolveDashboardRole(user)
      setStoredActiveRole(this.activeRole)
    },
    syncRouteRole(path: string) {
      const routeRole = dashboardRoleFromPath(path)
      if (routeRole) {
        this.setActiveRole(routeRole)
      }
    },
    async hydrateSession() {
      this.hydrateActiveRole()
    ...
    async login(payload: LoginPayload, options: { rememberMe?: boolean } = {}) {
      ...
      this.syncActiveRoleFromUser(this.user)
      this.initialized = true
      return result
    },
    ...
    async fetchMe() {
      ...
      this.user = user
      this.syncActiveRoleFromUser(user)
      return user
    },
    ...
    async changePassword(currentPassword: string, newPassword: string) {
      ...
      const { api } = useApi()
      try {
        return await api<{ detail: string }>(API.auth.changePassword, {
          method: 'POST',
